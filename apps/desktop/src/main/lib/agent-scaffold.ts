...
// The bridge imports go through in-worktree symlinks (.claude/memory and
// .claude/USER.md, created by ensureMemoryBridgeLinks) instead of absolute
// paths into the agent home. Claude Code treats symlinked files inside the
// project as internal, so this avoids the "Allow external CLAUDE.md file
// imports?" dialog that otherwise interrupts an agent's very first session —
// and where a "No" would silently strip the persona and write-back protocol.
const CLAUDE_BRIDGE = `@.claude/memory/AGENT.md
@.claude/USER.md
@.claude/memory/.writeback-protocol.md
<!-- MEMORY.md is loaded via Claude Code native auto-memory (autoMemoryDirectory). -->
`;

// The pre-2026-07-15 bridge (absolute external imports). Kept verbatim so the
// scaffold can recognize an untouched legacy bridge and upgrade it in place;
// a bridge the user edited is never rewritten.
const LEGACY_CLAUDE_BRIDGE = `@{{agent_home}}/memory/AGENT.md
@{{shared_user_md}}
...
function reflectHookScript(agentHome: string, userName: string): string {
	...
	// Kept short on purpose: Claude Code prints this whole string into the
	// transcript as a "Stop hook" banner, so a long reason reads as noise at the
	// end of every session. The full instructions (targets, anti-patterns,
	// budgets) are already in context via the write-back protocol import.
	const reason =
		`[session reflection] Before finishing, run the Session-end reflection ` +
		`from your write-back protocol: save anything durable from this ` +
		`conversation (${userName}'s preferences → shared USER.md, environment ` +
		`facts → MEMORY.md, workflow corrections → the governing skill). If ` +
...
/**
 * Ensure linkPath is a symlink to targetPath.
 *
 ...
 *   real dir is replaced with the link). A stray regular file is replaced.
 * Best-effort: never throws, so it can sit on the terminal-launch path.
 */
function ensureSymlinkAt(
	linkPath: string,
	targetPath: string,
	kind: "dir" | "file",
): void {
	try {
		...
		if (stat?.isSymbolicLink()) {
			...
			if (resolve(dirname(linkPath), target) === resolve(targetPath)) return;
			unlinkSync(linkPath);
			...
			if (hasContent) {
				console.warn(
					`[agent-scaffold] ${linkPath} is a real directory with content; ` +
						`leaving it in place (user-owned). ${targetPath} will not be ` +
						`reachable through this link.`,
				);
				return;
			}
			rmdirSync(linkPath);
		...
		console.warn(`[agent-scaffold] Failed to ensure link ${linkPath}:`, error);
	}
}
...
export function ensureClaudeSkillsLink(
	...
	try {
		...
		ensureSymlinkAt(join(claudeDir, "skills"), skillsDir, "dir");
	} catch (error) {
		console.warn(
...
/**
 * Ensure the in-worktree links the CLAUDE.md bridge imports resolve through:
 *   <worktree>/.claude/memory  → <agent-home>/memory      (dir)
 *   <worktree>/.claude/USER.md → shared USER.md            (file)
 * Claude Code treats symlinked files inside the project as internal, so
 * imports via these links don't trigger the external-imports dialog. Called
 * at scaffold time AND before every Claude session launch, like the skills
 * link. Best-effort: never throws.
 */
export function ensureMemoryBridgeLinks(
	agentId: string,
	worktreePath?: string,
): void {
	try {
		...
		ensureSymlinkAt(join(claudeDir, "memory"), memoryDir, "dir");
		ensureSymlinkAt(
			join(claudeDir, "USER.md"),
			getSharedUserProfilePath(),
			"file",
		);
	} catch (error) {
		console.warn(
			"[agent-scaffold] Failed to ensure memory bridge links:",
			error,
		);
	}
}
...
export function scaffoldAgentMemory({
	...
	// The links must exist before the bridge is useful (its imports resolve
	// through them).
	ensureMemoryBridgeLinks(agentId, worktreePath);
	const bridgePath = join(worktreePath, "CLAUDE.md");
	const legacyBridge = sub(LEGACY_CLAUDE_BRIDGE, vars);
	if (
		existsSync(bridgePath) &&
		readFileSync(bridgePath, "utf8") === legacyBridge
	) {
		// An untouched pre-symlink bridge: upgrade it in place so existing
		// agents also stop hitting the external-imports dialog. Anything the
		// user edited is left alone.
		writeFileSync(bridgePath, sub(CLAUDE_BRIDGE, vars), "utf8");
	} else {
		writeIfMissing(bridgePath, sub(CLAUDE_BRIDGE, vars));
	}
	...
	// Always overwrite: the hook is machine-owned generated code (its header
	// says so), and the launch-time backfill is how existing agents pick up
	// hook fixes. writeIfMissing here would freeze every agent on the version
	// current at its creation date.
	writeFileSync(
		reflectHookPath,
