...
const TEST_ROOT = path.join(
	tmpdir(),
	`jkoczeniakell-wrappers-${process.pid}-${Date.now()}`,
);
...
describe("shell-wrappers", () => {
	...
	it("creates zsh wrappers with interactive .zlogin sourcing and command shims", () => {
		...
		expect(zshenv).toContain('source "$_roster_home/.zshenv"');
		expect(zshenv).toContain(`export ZDOTDIR="${TEST_ZSH_DIR}"`);

		expect(zshrc).toContain("_roster_prepend_bin()");
		...
		expect(zlogin).toContain('source "$_roster_home/.zlogin"');
		expect(zlogin).toContain("_roster_prepend_bin()");
		expect(zlogin).toContain(`claude() { "${TEST_BIN_DIR}/claude" "$@"; }`);
		expect(zlogin).toContain(`codex() { "${TEST_BIN_DIR}/codex" "$@"; }`);
	...
	it("creates bash wrapper with command shims and idempotent PATH prepend", () => {
		...
		expect(rcfile).toContain("_roster_prepend_bin()");
	...
	describe("fish shell", () => {
		it("uses --init-command to prepend BIN_DIR to PATH for fish", () => {
			...
			expect(args).toEqual([
				"-l",
				"--init-command",
				`set -l _roster_bin "${TEST_BIN_DIR}"; contains -- "$_roster_bin" $PATH; or set -gx PATH "$_roster_bin" $PATH`,
			]);
		});
		...
		it("escapes fish init-command BIN_DIR safely", () => {
			...
			expect(args).toEqual([
				"-l",
				"--init-command",
				`set -l _roster_bin "/tmp/with space/quote\\"buck\\$slash\\\\bin"; contains -- "$_roster_bin" $PATH; or set -gx PATH "$_roster_bin" $PATH`,
			]);
		});
	});
});
