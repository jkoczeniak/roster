 ...
 */
export function getRosterHomeDir(): string {
	return process.env[ADE_HOME_DIR_ENV] || join(homedir(), ROSTER_DIR_NAME);
}

export const ROSTER_HOME_DIR = getRosterHomeDir();
process.env[ADE_HOME_DIR_ENV] = ROSTER_HOME_DIR;

export const ROSTER_HOME_DIR_MODE = 0o700;
export const ROSTER_SENSITIVE_FILE_MODE = 0o600;

export function ensureRosterHomeDirExists(): void {
	if (!existsSync(ROSTER_HOME_DIR)) {
		mkdirSync(ROSTER_HOME_DIR, {
			recursive: true,
			mode: ROSTER_HOME_DIR_MODE,
		});
	}
...
export const APP_STATE_PATH = join(ROSTER_HOME_DIR, "app-state.json");

// Window geometry state (separate from UI state - main process only, sync I/O)
export const WINDOW_STATE_PATH = join(ROSTER_HOME_DIR, "window-state.json");
