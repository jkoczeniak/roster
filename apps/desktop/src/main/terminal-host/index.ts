 ...
 * - Socket: ~/.roster/terminal-host.sock
...
// ROSTER_DIR_NAME is imported from shared/constants for multi-worktree support
// This allows workspace-specific home directories (e.g., ~/.roster-my-feature)
...
async function startServer(): Promise<void> {
	// Ensure roster directory exists with proper permissions
	if (!existsSync(ROSTER_HOME_DIR)) {
		mkdirSync(ROSTER_HOME_DIR, { recursive: true, mode: 0o700 });
		log("info", `Created directory: ${ROSTER_HOME_DIR}`);
	}
...
async function main() {
	...
	log("info", `Home directory: ${ROSTER_HOME_DIR}`);
