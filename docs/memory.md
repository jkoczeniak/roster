...
AGENT.md also carries a **`## Tools` section** — the explicit list of tools and connectors this agent's tasks require, with a one-line "use it for" note each. Roster appends an entry automatically when you wire a connector in the Connectors panel; edit the notes to sharpen when the agent should reach for each tool. This is *guidance* (the persona is read every session, which is what steers tool choice); for hard *enforcement* — allowing or blocking specific tools outright — use the CLI's own permission rules in the worktree's `.claude/settings.json` (`permissions.allow` / `permissions.deny`, e.g. `mcp__atlassian__*`), which Claude Code enforces mechanically.
